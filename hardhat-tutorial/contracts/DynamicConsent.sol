...
contract DynamicConsent {
    ...
    mapping(string => uint256[]) private clusteringIndex;
    ...
    */
    ...
    function generateClusteringKey(string[] calldata _categories, string[] calldata _elements, uint256 _studyID) internal pure returns (string memory) {
        // Generate a unique string key based on categories, elements, and studyID
        return string(abi.encodePacked(keccak256(abi.encodePacked(_categories)), "|", keccak256(abi.encodePacked(_elements)), "|", _studyID));
    }
    ...
    */
    function storeRecord(uint256 _patientID, uint256 _studyID, uint256 _recordTime, string[] calldata _patientCategoryChoices, string[] calldata _patientElementChoices) public {
        ...
        // Update clustering index with patientID
        string memory clusteringKey = generateClusteringKey(_patientCategoryChoices, _patientElementChoices, _studyID);
        clusteringIndex[clusteringKey].push(_patientID);
    }
    

    function storeConsentFromJSON(
    uint256 _patientID,
    uint256 _studyID,
    uint256 _recordTime,
    ...
    string[] calldata _elementChoices
        ...
        // Call the existing storeRecord function to store the consent
        storeRecord(_patientID, _studyID, _recordTime, _categoryChoices, _elementChoices);
    }
